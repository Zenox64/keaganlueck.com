...
function inOrder() {
  document.getElementById("myPicture").src = myPix.sort;
}
...
function changingText() {
  ...
  // Stop the last one and start a new one
  clearInterval(cycle);
  choosePic();
}

function choosePic() {
  // Assigned the setInterval to a variable to be able to stop it
  cycle = setInterval(() => {
    var randomNum = Math.floor(Math.random() * myPix.length);
    document.getElementById("myPicture").src = myPix[randomNum];
  }, text);
}
function chooseTechPic() {
  // Assigned the setInterval to a variable to be able to stop it
  cycle = setInterval(() => {
    var randomNum = Math.floor(Math.random() * myTechPix.length);
    document.getElementById("myPicture").src = myTechPix[randomNum];
  }, text);
}
